...
def clip_boxes_graph(boxes, window):
    ...
    window: [4] in the form y1, x1, z1, y2, x2, z2
    """
    # Split corners
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        height, width, depth = self.config.IMAGE_SHAPE[:3]
        window = np.array([0, 0, 0, height, width, depth]).astype(np.float32)
        ...
        normalized_boxes = boxes / np.array([[height, width, depth, height, width, depth]])
        ...
        def nms(normalized_boxes, scores):
            ...
            proposals = tf.concat([proposals, tf.zeros([padding, 6])], 0)
            return proposals
        proposals = utils.batch_slice([normalized_boxes, scores], nms,
                                       self.config.IMAGES_PER_GPU)
        return proposals

    def nms(self, normalized_boxes,scores, proposal_count, nms_threshold):
        keep = self.py_cpu_nms(normalized_boxes, scores,  nms_threshold)
    ...
    #normalized boxes should have shape [Batch, N, 6] and scores should have [Batch, N, 1]
    def py_cpu_nms(self,normalized_boxes, scores,  thresh):
        """Pure Python NMS baseline."""
        y1 = normalized_boxes[:, 0]
        x1 = normalized_boxes[:, 1]
        z1 = normalized_boxes[:, 2]
        y2 = normalized_boxes[:, 3]
        x2 = normalized_boxes[:, 4]
        z2 = normalized_boxes[:, 5]


        volume = (y2 - y1 + 1)  * (x2 - x1 + 1) * (z2 - z1 +1)
        order = scores.argsort()[::-1]
        #order = tf.gather(scores, tf.nn.top_k(scores[:, 0], k=scores.shape[0]).indices)
        #order = tf.contrib.framework.sort(scores, axis = 1, direction='DESCENDING',name="sort proposals")
...
class PyramidROIAlign(KE.Layer):
    ...
    Params:
    - pool_shape: [height, width, depth] of the output pooled regions. Usually [7, 7, 7]
    - image_shape: [height, width, depth, chanells]. Shape of input volume in voxels
    ...
    def call(self, inputs):
        ...
        # e.g. a 224x224 ROI (in pixels) maps to P4. TODO I added 128 instead of 224 because my input is at most 128 pixels. see paper Feature Pyramid Networks for Object Detection
        ...
        for i, level in enumerate(range(2, 5)):
            ...
            pooled.append(tf.image.crop_and_resize3d(feature_maps[i], level_boxes, box_indices, self.pool_shape,method="trilinear"))
    ...
    def compute_output_shape(self, input_shape):
        res = input_shape[0][:2]
        res = res + self.pool_shape
        res = res + (input_shape[1][-1], )
        return res
...
def detection_targets_graph(proposals, gt_boxes, config):
    ...
    gt_boxes: [MAX_GT_INSTANCES, (y1, x1,z1 y2, x2, z2 class_id)] in
    ...
    deltas: [TRAIN_ROIS_PER_IMAGE, NUM_CLASSES, (dy, dx, dz, log(dh), log(dw), log(dd))]
    ...
    roi_y1, roi_x1, roi_z1, roi_y2, roi_x2, roi_z2 = tf.split(rois, 6, axis=1)
    box_y1, box_x1, box_z1, box_y2, box_x2, box_z2, class_ids = tf.split(boxes, 7, axis=1)
    y1 = tf.maximum(roi_y1, box_y1)
    x1 = tf.maximum(roi_x1, box_x1)
    z1 = tf.maximum(roi_z1, box_z1)
    ...
    union = tf.identity(union, name="UNION")
    ...
    positive_rois = tf.gather(proposals, positive_indices,name="positive_rois_FOR_DELTA")
    negative_rois = tf.gather(proposals, negative_indices)

    # Assign positive ROIs to GT boxes.
    positive_overlaps = tf.gather(overlaps, positive_indices)
    roi_gt_box_assignment = tf.argmax(positive_overlaps, axis=1)
    roi_gt_boxes = tf.gather(gt_boxes, roi_gt_box_assignment,name="ROI_GT_BOXES_FOR_DELTA")

    # Compute bbox refinement for positive ROIs
    deltas = utils.box_refinement_graph(positive_rois, roi_gt_boxes[:,:6])
...
class DetectionTargetLayer(KE.Layer):
    ...
    proposals: [batch, N, (y1, x1, z1, y2, x2, z2)] in normalized coordinates. Might
               be zero padded if there are not enough proposals.
    gt_boxes: [batch, MAX_GT_INSTANCES, (y1, x1, z1, y2, x2, z2, class_id)] in
              normalized coordinates.
    ...
    target_deltas: [batch, TRAIN_ROIS_PER_IMAGE, NUM_CLASSES, 
                    (dy, dx, dz, log(dh), log(dw), log(dd), class_id)]
...
class DetectionLayer(KE.Layer):
    ...
    Returns:
    [batch, num_detections, (y1, x1, z1, y2, x2, z2, class_score)] in voxels
    """
...
# rpn = build_rpn_model(config.RPN_ANCHOR_STRIDE, 2
#                       len(config.RPN_ANCHOR_RATIOS), 3: (len([0.5, 1, 2]))
#                        256)
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    rpn_probs: [batch, W, W, D, 2] Anchor classifier probabilities.
    rpn_bbox: [batch, H, W, D, (dy, dx, dz, log(dh), log(dw), log(dd))] Deltas to be
                applied to anchors.
    """
...
def fpn_classifier_graph(rois, feature_maps,
    ...
    Returns:
        ...
        bbox_deltas: [N, (dy, dx, dz, log(dh), log(dw), log(dd))] Deltas to apply to 
                     proposal boxes
    """
    # ROI Pooling
    ...
    # BBox head
    # [batch, boxes, num_classes * (dy, dx, dz,  log(dh), log(dw), log(dd))]
...
def viz_anchors_gt(overlaps, gt, anchors):
    ...
    for i in range(len(anchors) + 1):
        ...
        if i == len(anchors):
            ...
            if overlaps[i] > 0.3 or overlaps[i]<0.1: continue
...
def build_rpn_targets(image_shape, anchors, gt_boxes, config):
    ...
    gt_boxes: [num_gt_boxes, (y1, x1, z1, y2, x2, z2, class_id)]
    ...
    rpn_match[anchor_iou_max < config.ANCHOR_IOU_NEG_TRESH] = -1
    # 2. Set an anchor for each GT box (regardless of IoU value).
    # TODO: If multiple anchors have the same IoU match all of them
    gt_iou_argmax = np.argmax(overlaps, axis=0)
    rpn_match[gt_iou_argmax] = 1
    # 3. Set anchors with high overlap as positive.
    rpn_match[anchor_iou_max >= config.ANCHOR_IOU_POS_TRESH] = 1
    ...
    for i, a in zip(ids, anchors[ids]):
        ...
        gt = gt_boxes[anchor_iou_argmax[i], :6]
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # [batch, MAX_GT_INSTANCES, (y1, x1,z1, y2, x2,z2 class_id)] in image coordinates. the order actually does not matter
            input_gt_boxes = KL.Input(shape=[None, 7], name="input_gt_boxes", dtype=tf.int32)
        ...
        P4 = KL.Conv3D(192, (1, 1, 1), name='fpn_c4p4')(C4)
        P3 = KL.Add(name="fpn_p4add")([
            KL.UpSampling3D(size=(2, 2, 2), name="fpn_p5upsampled")(P4),
            KL.Conv3D(192, (1, 1, 1), name='fpn_c3p3')(C3)])
        P2 = KL.Add(name="fpn_p3add")([
            KL.UpSampling3D(size=(2, 2, 2), name="fpn_p4upsampled")(P3),
            KL.Conv3D(192, (1, 1, 1), name='fpn_c2p2')(C2)])

        # Attach 3x3 conv to all P layers to get the final feature maps.
        P2 = KL.Conv3D(192, (3, 3, 3), padding="SAME", name="fpn_p2")(P2)
        P3 = KL.Conv3D(192, (3, 3, 3), padding="SAME", name="fpn_p3")(P3)
        P4 = KL.Conv3D(192, (3, 3, 3), padding="SAME", name="fpn_p4")(P4)
        ...
        # layer_outputs contains a list of 5 layers. each layer has 3 tensors ( logits, probabilities and bboxes)
        ...
        #these above have the following shapes
        #[Batch, num_rois, 2]
        #[Batch, num_rois, 2]
        #[Batch, num_rois, 6]
        ...
        # Generate proposals
        # Proposals are [N, (y1, x1, z1, y2, x2, z2)] in normalized coordinates.
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                target_rois = KL.Lambda(lambda x: K.cast(x, tf.float32) / image_scale[:6])(input_rois)
            else:
